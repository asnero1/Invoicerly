...
export default function VTOLogger() {
  ...
  const handleTextChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    else if (value.toLowerCase().includes('content')) setSuggestedTag('Creative');
      ...
      <div className="border p-6 rounded shadow bg-white space-y-6">
        ...
        {suggestedTag && !confirmedTag && (
          <div className="text-sm text-gray-600">
            Suggested folder: <span className="font-semibold">{suggestedTag}</span>
            <Button onClick={handleConfirmTag} className="ml-2 px-2 py-1 text-sm">
              Confirm
            </Button>
          </div>
        )}

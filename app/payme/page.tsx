...
export default function PayMePage() {
  ...
  const filtered = tasks.filter((task) => {
    const matchSearch = (task.title || task.description || '').toLowerCase().includes(search.toLowerCase());
    const matchPaid =
      filterPaid === 'all' ||
      (filterPaid === 'paid' && task.paid) ||
      (filterPaid === 'unpaid' && !task.paid);
    const matchClient = filterClient === 'all' || task.client === filterClient;
    const matchWeek = !filterWeek || (new Date(task.date) >= weekStart && new Date(task.date) <= weekEnd);
  ...
  const revenueThisWeek = tasks
    .filter((t) => t.paid && new Date(t.date) >= weekStart && new Date(t.date) <= weekEnd)
  ...
  const revenueThisMonth = tasks
    .filter((t) => {
      ...
      const now = new Date();
      return t.paid && date.getMonth() === now.getMonth() && date.getFullYear() === now.getFullYear();
    })
    .reduce((sum, t) => sum + (t.amount || 0), 0);
  ...
  tasks.forEach((t) => {
    if (t.paid) {
      revenueByClient[t.client] = (revenueByClient[t.client] || 0) + (t.amount || 0);
    }
  });

  const markAsPaid = async (taskId: string) => {
    try {
      const updated = tasks.map((t) => (t.id === taskId ? { ...t, paid: true } : t));
      setTasks(updated);
  ...
  const exportReceiptsZip = async () => {
    ...
    for (const task of receiptTasks) {
      try {
        ...
        zip.file(task.receiptUrl!.split('/').pop() || `receipt-${task.id}`, blob);
      } catch {
      ...
      <div className="flex flex-col md:flex-row justify-between gap-4 items-start md:items-start">
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}>
          <div className="space-y-1 text-sm text-gray-700">
            <div><strong>{paidCount}</strong> Paid, <strong>{unpaidCount}</strong> Unpaid</div>
            <div>💰 <strong>This Week:</strong> ${revenueThisWeek.toFixed(2)}</div>
            <div>📅 <strong>This Month:</strong> ${revenueThisMonth.toFixed(2)}</div>
            <div>📊 <strong>By Client:</strong></div>
            <ul className="ml-4 list-disc">
              {Object.entries(revenueByClient).map(([client, total]) => (
                <li key={client}>{client}: ${total.toFixed(2)}</li>
              ))}
            </ul>
          </div>
        </motion.div>
        ...
        <div className="space-x-2 mt-4 md:mt-0">
          ...
          <Button onClick={exportReceiptsZip} variant="secondary">Export Receipts</Button>
          <Button onClick={exportAllCSV} variant="secondary">Export All</Button>
        </div>
      </div>
      ...
      >
        {filtered.map((task) => (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
          >
            <p><strong>{task.title || task.description || '—'}</strong></p>
            ...
            <p className={`font-semibold ${task.paid ? 'text-green-600' : 'text-yellow-600'}`}>
              {task.paid ? '✅ Paid' : '❌ Unpaid'}
            </p>
            <p className="text-sm">Amount: ${task.amount?.toFixed(2) || '0.00'}</p>
            {task.receiptUrl && (
              <img
                src={task.receiptUrl}
                alt="Receipt"
            ...
            {task.paid && (
              <button
                className={`text-sm px-3 py-1 rounded border ${
                  likes[task.id] ? 'bg-green-100 border-green-400' : 'border-gray-300'
                }`}

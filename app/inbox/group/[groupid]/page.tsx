...
export default function GroupChatPage() {
  ...
  const loadMessages = async () => {
    ...
    setMessages(relevant.sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime()));
  };
  ...
  return (
    <motion.div className="p-6 max-w-2xl mx-auto" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      {group ? (
        <>
          <h1 className="text-2xl font-bold mb-2">👥 {group.name}</h1>
          <p className="text-sm text-gray-500 mb-4">Members: {group.members.join(', ')}</p>
            ...
            {messages.map((msg, i) => (
              <motion.div
                ...
                className={`p-3 rounded-xl shadow-sm w-fit max-w-[80%] ${
                  msg.sender === CURRENT_USER ? 'bg-blue-100 ml-auto text-right' : 'bg-gray-100'
                }`}
              >
                <p className="text-sm font-semibold text-gray-600 mb-1">{msg.sender}</p>
